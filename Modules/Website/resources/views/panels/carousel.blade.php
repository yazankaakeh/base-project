@php
    /**
     * Modern Carousel Panel Component (Panel Model)
     *
...
<section id="panel-{{ $panel->id }}"
         class="panel-section carousel-panel carousel-style-{{ $carouselStyle }} {{ $heightClass }} position-relative"
         dir="{{ $isRtl ? 'rtl' : 'ltr' }}">
                ...
                <div class="swiper-wrapper">
                    @forelse($items as $item)
                        @php
                            ...
                            $imageUrl = $item->getFirstMediaUrl('item_image') ?: asset('assets/img/placeholders/carousel-placeholder.jpg');
                            ...
                            >
                                ...
                                @if($carouselStyle === 'cards')
                                    <div class="card carousel-card h-100 border-0 shadow-lg overflow-hidden">
                                        ...
                                        <div class="card-body p-4">
                                            ...
                                            @endif
                                            @if($itemTitle)
                                                <h4 class="card-title mb-2">{{ $itemTitle }}</h4>
                                            @endif
                                            @if($itemContent)
                                                <p class="card-text text-muted mb-3">{{ Str::limit($itemContent, 120) }}</p>
                                            @endif
                                            @if($buttonText)
                                                <a href="{{ $buttonUrl }}" class="btn btn-primary btn-sm">
                                                    {{ $buttonText }}
                                                    <i class="ti tabler-arrow-{{ $isRtl ? 'left' : 'right' }} ms-1"></i>
                                                </a>
                                            @endif
                                        </div>
                                    </div>
                                ...
                                @elseif($carouselStyle === 'minimal')
                                    <div class="carousel-minimal-slide">
                                        ...
                                        <div class="carousel-minimal-content mt-4 text-center">
                                            @if($itemTitle)
                                                <h4 class="mb-2">{{ $itemTitle }}</h4>
                                            @endif
                                            @if($itemContent)
                                                <p class="text-muted">{{ Str::limit($itemContent, 100) }}</p>
                                            @endif
                                        </div>
                                    </div>
                                            ...
                                            @if($buttonText)
                                                <div class="carousel-hero-cta animate-fadeInUp animation-delay-3">
                                                    <a href="{{ $buttonUrl }}" class="btn btn-primary btn-lg px-5 shadow-lg">
                                                        {{ $buttonText }}
                                                        <i class="ti tabler-arrow-{{ $isRtl ? 'left' : 'right' }} ms-2"></i>
                                                    </a>
                                                </div>
                                            @endif
                                        </div>
                                ...
                                @else
                                    <div class="carousel-default-slide position-relative rounded-4 overflow-hidden">
                                        ...
                                        @endif
                                        @if($itemTitle || $itemContent || $buttonText)
                                            <div class="carousel-default-caption position-absolute bottom-0 start-0 end-0 p-4 text-white">
                                                ...
                                                @endif
                                                @if($itemContent)
                                                    <p class="mb-3 opacity-90">{{ Str::limit($itemContent, 150) }}</p>
                                                @endif
                                                @if($buttonText)
            ...
            @if($showNavigation && $items->count() > 1)
                <div class="carousel-navigation">
                    <button type="button"
                            ...
                            id="{{ $carouselId }}-prev">
                        <i class="ti tabler-chevron-{{ $isRtl ? 'right' : 'left' }} ti-md"></i>
                    </button>
                    <button type="button"
                            class="carousel-nav-btn carousel-nav-next btn btn-icon btn-primary rounded-circle shadow-lg"
                            id="{{ $carouselId }}-next">
                        <i class="ti tabler-chevron-{{ $isRtl ? 'left' : 'right' }} ti-md"></i>
                    </button>
                </div>
            @endif
        </div>
...
.carousel-pagination .swiper-pagination-bullet {
    width: 10px;
    height: 10px;
    background: var(--panel-primary, var(--bs-primary));
    opacity: 0.3;
    transition: all 0.3s ease;
}
...
.animate-fadeInUp {
    animation: carouselFadeInUp 0.8s ease forwards;
    opacity: 0;
}
...
.carousel-overlay.overlay-primary {
    background: linear-gradient(to top, rgba(var(--panel-primary-rgb, 30, 170, 231), 0.8) 0%, rgba(var(--panel-primary-rgb, 30, 170, 231), 0.4) 50%, rgba(var(--panel-primary-rgb, 30, 170, 231), 0.1) 100%);
}
