...
<!-- Page Scripts -->
@section('page-script')
    @includeIf('doctor::doctor.medicalSpecialty.modals.createModal')
    @includeIf('doctor::doctor.medicalSpecialty.modals.editModal')
    ...
    --}}
@endsection
@section('title', trans('customer.sidebar.medicalSpecialty'))
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between pb-2 mb-1">
                            <h5 class="">{{trans('customer.sidebar.medicalSpecialty')}}</h5>
                            <h5 class="">
                                @can('doctor.medicalSpecialty.store')
                                    <button type="button"
                                ...
                                <div class="table-responsive text-nowrap">
                                    <table class="table datanew">
                                        ...
                                        <tr>
                                            ...
                                            <th>{{trans('doctor::doctor.medicalSpecialty.name')}}</th>
                                            <th>{{trans('doctor::doctor.medicalSpecialty.code')}}</th>
                                            <th>{{trans('customer.account.status')}}</th>
                                            <th>{{trans('admin.audits.action')}}</th>
                                        </tr>
                                        </thead>
                                        <tbody class="table-border-bottom-0">
                                        @foreach($data as $medicalSpecialty)
                                            <tr>
                                                <td>{{$medicalSpecialty->id}}</td>
                                                <td>{{$medicalSpecialty->name}}</td>
                                                <td>{{$medicalSpecialty->code}}</td>
                                                <td>
                                                    <span class="badge text-bg-{{$medicalSpecialty->is_active->class()}} me-1">{{$medicalSpecialty->is_active->label()}} </span>
                                                </td>
                                                <td class="action-table-data">
                                                    <div class="edit-delete-action">
                                                        @can('doctor.medicalSpecialty.update')
                                                            <a type="button" data-bs-toggle="modal"
                                                               data-bs-target="#editModal"
                                                               class="me-2 btn btn-outline-primary text-primary p-2 btn-sm EditModalBTN"
                                                               data-id="{{$medicalSpecialty->id}}"
                                                               data-code="{{$medicalSpecialty->code}}"
                                                               data-name='@json($medicalSpecialty->getTranslations('name'))'
                                                               data-active="{{$medicalSpecialty->is_active}}">
                                                                <i data-feather="edit"
